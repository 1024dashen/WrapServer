...
auth.post('/register', async (c) => {
    ...
    return c.json({
        ...
        id,
        token: newToken,
        user: { id, email, username, role: '普通管理员', status: 'active' },
    })
})
...
auth.get('/me', authMiddleware, async (c) => {
    ...
    const roleResult = db.exec('SELECT permissions FROM roles WHERE name = ?', [
        dbUser.role,
    ])
    const permissions =
        roleResult.length > 0 && roleResult[0].values.length > 0
