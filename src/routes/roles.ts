...
roles.get('/', async (c) => {
  ...
  const roles = result.length > 0 ? result[0].values.map(row => {
    const obj: any = {};
...
roles.get('/:id', async (c) => {
  ...
  const role: any = {};
  columns.forEach((col, i) => { role[col] = values[i]; });
...
roles.post('/', async (c) => {
  ...
  db.run('INSERT INTO roles (name, description, permissions) VALUES (?, ?, ?)', [name, description || '', JSON.stringify(permissions || [])]);
  saveDb();
