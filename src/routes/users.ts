...
users.get('/', async (c) => {
  ...
  const result = db.exec('SELECT id, email, username, role, status, created_at FROM users ORDER BY id');
...
users.get('/:id', async (c) => {
  ...
  const db = await getDb();
  const result = db.exec('SELECT id, email, username, role, status, created_at FROM users WHERE id = ?', [id]);
...
users.post('/', async (c) => {
  ...
  db.run(
    ...
    [email, username, hashedPassword, role || '运营人员', status || 'active']
  );
  saveDb();
